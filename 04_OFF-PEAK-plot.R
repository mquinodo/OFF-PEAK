...
# end=64694496
# side=20
# data="/home/mquinodo/SYNO/scripts_NGS_analysis/OFF-PEAK-train4/unsolved-21-v13/05_RData-files/data-CHbasl0212.RData"
# out="/home/mquinodo/SYNO/scripts_NGS_analysis/OFF-PEAK-train4/unsolved-21-v13/CHbasl0212test"
# databasefile="/home/mquinodo/SYNO/scripts_NGS_analysis/OFF-PEAK-train4/refs/data-hg19.RData"
# UseCano=TRUE
...
plotcnv <- function(chr,begin,end,ID,data,pdf,side,offtar,UseCano) {
...

  {
    ...
    for(i in 1:dim(pos)[1]){
      # corrected counts
      pos[i,4]=as.numeric(d[which(as.numeric(d[,2])==pos[i,1]),6]) # ID
      pos[i,5]=as.numeric(d[which(as.numeric(d[,2])==pos[i,1]),7]) # mean
      pos[i,6]=as.numeric(d[which(as.numeric(d[,2])==pos[i,1]),8]) # SD
...
pdf=paste(out1,"/",ID,"-",chr,"-",begin,"-",end,".pdf",sep="")
plotcnv(chr,begin,end,ID,data,pdf,side,TRUE,UseCano)

#plotting targets and antitargets
print("Doing plot with on-targets only")
pdf=paste(out2,"/",ID,"-",chr,"-",begin,"-",end,"_on-targets.pdf",sep="")
