...
nametarget=Twist_ComprehensiveExome_targets_hg19
...
folder=$here/scripts_NGS_analysis/OFF-PEAK
batch=unsolved-2021
bedFile=$here/NGS/Twist/Twist_ComprehensiveExome_targets_hg19.bed
grep -v -P "chrUn|chrX|chrY|_gl00" $bedFile > $bedFile.cano.bed
...
mkdir -p $output
sed 's/chr//g' $targetfile | grep -v -P "_|Y|Un" | awk -F"\t" '{if($3-$2>10) print $0}' > $targetfile.temp
cat $script/header.txt $targetfile.temp > $output/probes.txt
rm $targetfile.temp
cp $here/NGS/conifer/probes.txt $output/probes2.txt
...
svd=4 # Here look at the scree plot to adjust
...
output=$folder/$batch/controlFREEC
mkdir -p $output
a=$(ls $here2/*bam | cut -d"/" -f5 | cut -d"." -f1 | tr '\n' ',')
b=${a::-1}
IFS=',' read -r -a patients <<< "$b"
...
do
	sed "s|oouutt|$output|g" $script/config_default-2021.txt | sed "s|iinn|$here2\/$patient.dup.BQSR.bam|g" > $output/$patient.config.txt
...
do
	...
	awk -F"\t" -v patient="$patient" '{print "chr" $1 "\t" $2 "\t" $3 "\t" patient "\t" $4 "\t" $4}' $output/$patient.dup.BQSR.bam_CNVs > $output/$patient.dup.BQSR.bam_CNVs.p.value.pro.txt 
done

cat $output/*.dup.BQSR.bam_CNVs.p.value.pro.txt | awk -F"\t" '{if($3>$2) print $0}' | grep -v "WilcoxonRankSumTestPvalue" > $output/$batch.controlFREEC.out.tsv
...
targets=$here/refs/Twist_ComprehensiveExome_targets_hg19.bed
...
here=/home/mquinodo/SYNO/scripts_NGS_analysis/OFF-PEAK-train5
batch=unsolved-2021-small
script=$here/scripts/02_bam-count.sh
work=$here/$batch
listBAM=$work/$batch.bam_list.txt
targetsBED=$here/refs/Twist-CE-small.bed
mosdepth=/home/mquinodo/mosdepth
bash $script --work $work --listBAM $listBAM --mosdepth $mosdepth --targetsBED $targetsBED
...
side=20
data=$here/unsolved-2021-small/05_RData-files/data-$pat.RData
out=$here/unsolved-2021-small/$pat
databasefile=$here/refs/data-hg19.RData
...
side=20
data=$here/unsolved-2021/05_RData-files/data-$pat.RData
out=$here/unsolved-2021/$pat
databasefile=$here/refs/data-hg19.RData
Rscript $script --ID $pat --chr $chr --begin $begin --end $end --side $side --data $data --out $out --databasefile $databasefile
