...
nametarget=ICR96
targetfile=$here/WES/ICR96/03_info/ICR96.bed
...
a=$(ls $here/$lab/*/bam/*bam | cut -d"/" -f7 | tr '\n' ',')
b=${a::-1}
...
for pat in "${patients[@]}"
do
printf "$here/$lab/$pat/bam/$pat.dup.BQSR.bam " >> $output/combine-command.sh
done
...
for pat in "${patients[@]}"
do
	/home/mquinodo/cnvkit/cnvkit.py coverage $here/$lab/$pat/bam/$pat.dup.BQSR.bam $output/$nametarget.target.bed -o $output/$pat.targetcoverage.cnn &
	/home/mquinodo/cnvkit/cnvkit.py coverage $here/$lab/$pat/bam/$pat.dup.BQSR.bam $output/$nametarget.antitarget.bed -o $output/$pat.antitargetcoverage.cnn
done
...
cat $output/*.call2.tsv | grep -v -P "chrY|chromosome|gl" > $output/$batch.cnvkit.out.tsv
...
bedFile=$here/WES/ICR96/03_info/ICR96.bed
...
awk -F"\t" '{print $3 "\t" $5 "\t" $6 "\t" $2 "\t" $13 "\t" $12}' $output/output.tsv | tail -n+2 > $output/$batch.codex2.out.tsv


# CoNIFER

here=/home/mquinodo/SYNO
folder=$here/scripts_NGS_analysis/OFF-PEAK-train5
batch=ICR96
here2=/home/mquinodo/$batch
...
mkdir -p $output
# targetfile=$here/NGS/Twist/Twist_ComprehensiveExome_targets_hg19.bed
targetfile=$here/WES/ICR96/03_info/ICR96.bed
sed 's/chr//g' $targetfile | grep -v "_" > $targetfile.temp
...
# [INIT] Finished reading RPKM files. Total number of samples in experiment: 96 (0 failed to read properly)
...
# [RUNNING: chr1] Masking 19751 probes with median RPKM < 1.000000
...
# Closing remaining open files:/output/results/ICR96.svd1.analysis.hdf5...done
...
do
	sed "s|oouutt|$output|g" $script/config_default-ICR96.txt | sed "s|iinn|$here2\/$patient.dup.BQSR.bam|g" > $output/$patient.config.txt
...
# First to preprocess
# This only has to be run once per capture kit 
...
### Finally, postprocessing CNV calls
n=96
...
###### PART 0: preparation of files

NAS=/home/mquinodo/SYNO
batch=ICR96
work=$NAS/scripts_NGS_analysis/OFF-PEAK-train5/$batch
mkdir -p $work
ls /home/mquinodo/$batch/*.dup.BQSR.bam | awk -F"\t" '{split($1,a,"/"); split(a[5],b,"."); print $1 "\t" b[1]}' > $work/$batch.bam_list.txt
...
targets=/home/mquinodo/SYNO/WES/ICR96/03_info/ICR96.bed
...
targetsBED=$here/refs/ICR96-regions.bed
...
Rscript $script --output $here/${folder} --data $data --databasefile $databasefile &> $here/$folder/log.tsv
